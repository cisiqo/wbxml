defmodule WbxmlTest do
  ...
  test "decode wbxml response 4148" do
    ...
    xml = Wbxml.decode(bytes)
    assert xml == "<Provision><Status>1</Status><Policies><Policy><PolicyType>MS-EAS-Provisioning-WBXML</PolicyType><Status>1</Status><PolicyKey>787138854</PolicyKey></Policy></Policies></Provision>"
  end

  test "decode wbxml response 4569" do
    {ok, bytes} = File.read("test/wbxml_samples/response-4569.wbxml")
    xml = Wbxml.decode(bytes)
    assert xml == "<Sync><Collections><Collection><SyncKey>1437611349</SyncKey><CollectionId>RI</CollectionId><Status>1</Status></Collection></Collections></Sync>"
  end

  test "decode wbxml response 6318" do
    {ok, bytes} = File.read("test/wbxml_samples/response-6318.wbxml")
    xml = Wbxml.decode(bytes)
    assert xml == "<Sync><Collections><Collection><SyncKey>1219289161</SyncKey><CollectionId>2</CollectionId><Status>1</Status></Collection></Collections></Sync>"
  end

  test "decode wbxml response 6801" do
    {ok, bytes} = File.read("test/wbxml_samples/response-6801.wbxml")
    xml = Wbxml.decode(bytes)
    assert xml == "<Sync><Collections><Collection><SyncKey>500509522</SyncKey><CollectionId>7</CollectionId><Status>1</Status></Collection></Collections></Sync>"
  end
